...
export default function Column({
    ...
    function handleSave({ name, color }) {
        ...
        // fallbacks si no existe onUpdate
            ...
            if (typeof onRename === "function" && name && name !== col.name) onRename(col.id, name);
            if (typeof onColor === "function" && color && color !== col.color) onColor(col.id, color);
        }
        setShowEdit(false);
    }
    ...
    return (
        <>
            <div className="relative w-80 shrink-0 h-[calc(100dvh-9rem)] flex">
                <div className={`flex w-full flex-col rounded-3xl border-4 border-base-dark bg-${col.color.replace(".","-")} overflow-hidden`}>
                    ...
                    <div className="flex-1 min-h-0 pt-5 px-2">
                        <div ref={listRef} className="h-full overflow-y-auto pb-24 flex flex-col gap-4">
                            {cards.map((c, idx) => (
                                ...
                                className="self-center w-[88%] text-right"
                                draggable
                                onDragStart={(e)=>{
                                    e.dataTransfer.setData("text/plain", JSON.stringify({ fromColId: col.id, fromIndex: idx }));
                                ...
                                >
                                    <CardItem card={c} onEdit={() => onAddCard(col.id, idx, true)} />
                                </div>
                            ))}
